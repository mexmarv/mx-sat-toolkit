...
async def curp(nombre: str, apellido1: str, apellido2: str, fecha: Union[date, None], genero: str, codigo_edo: str):
    ...
    if codigo_edo.upper() not in entities.values():
        raise HTTPException(status_code=404, detail=codigo_edo +
                            " no existe. Los Validos son: " + json.dumps(entities))
    else:
        ciudad = list(entities.keys())[
            list(entities.values()).index(codigo_edo.upper())]
...
async def cfdi(rfce: str, rfcr: str, monto: str, folio: str):
    ...
    if (rfce != "") or (rfcr != "") or (monto != "") or (folio != ""):
        ...
        client = Client(
            'https://consultaqr.facturaelectronica.sat.gob.mx/ConsultaCFDIService.svc?WDSL')
