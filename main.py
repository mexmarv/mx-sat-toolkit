...
@app.get("/sat")
async def sat(rfce: str, rfcr: str, monto: str, folio: str):     
    obj_sat = {"rfce": rfce, "rfcr": rfcr, "monto": monto, "folio": folio}   
    
    if (rfce != "") or (rfcr != "") or (monto != "") or (folio != ""):
        client = Client('https://consultaqr.facturaelectronica.sat.gob.mx/ConsultaCFDIService.svc')
        result = client.service.Consulta(re=rfce, rr=rfcr, tt=monto, id=folio)
